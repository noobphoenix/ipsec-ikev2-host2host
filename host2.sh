...
ipsec pki --gen --type ecdsa --size 256 --outform pem > /etc/ipsec.d/private/host2.pem
ipsec pki --pub --type ecdsa --in \
/etc/ipsec.d/private/host2.pem | \
...
--san $HOST2 > /etc/ipsec.d/certs/host2.pem
...
echo "
$HOST2 $HOST1 : ECDSA /etc/ipsec.d/private/host2.pem
" > /etc/ipsec.secrets
